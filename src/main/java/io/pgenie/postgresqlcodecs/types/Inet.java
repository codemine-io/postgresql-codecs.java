...
/**
 * PostgreSQL {@code inet} type. IPv4 or IPv6 host address with optional subnet
 * mask.
 *
 * <p>
 * Holds an IPv4 or IPv6 host address, and optionally its subnet, all in one
 * field. The subnet is represented by the number of network address bits
 * present in the host address (the "netmask"). If the netmask is 32 and the
 * address is IPv4 (or 128 for IPv6), the value represents just a single host.
 *
 * <p>
 * Port of the Haskell {@code PostgresqlTypes.Inet} sum type.
 */
public sealed interface Inet permits Inet.V4, Inet.V6 {
    ...
    /**
     * Generates a random {@code Inet} value — either an IPv4 or IPv6 address
     * with a random host address and a netmask that covers the full valid
     * range.
     */
    static Inet generate(Random r) {
        if (r.nextBoolean()) {
            ...
            return new V6(r.nextInt(), r.nextInt(), r.nextInt(), r.nextInt(),
                    (byte) r.nextInt(0, 129));
        }
    }

    public static final Codec<Inet> CODEC = new Codec<Inet>() {
        ...
        public Inet decodeBinary(ByteBuffer buf, int length) throws Codec.ParseException {
            ...
            return switch (af) {
                ...
                default ->
                    throw new Codec.ParseException("Unknown inet address family: " + af);
            };
        }
        ...
        // -----------------------------------------------------------------------
        /**
         * Formats an {@link Inet} value as its canonical PostgreSQL text
         * representation.
         */
        ...
        /**
         * Parses an inet text value (with optional /n suffix) into an
         * {@link Inet}.
         */
        ...
        /**
         * Formats a 128-bit IPv6 address (stored as four 32-bit words) as
         * compressed text per RFC 5952, e.g. {@code ::1} instead of
        ...
        /**
         * Parses a compressed or full IPv6 address string into an array of 8
         * unsigned 16-bit groups.
         */
